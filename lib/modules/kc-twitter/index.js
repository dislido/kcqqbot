...
module.exports = class KCTwitter extends CQNode.Module {
  ...
  async onRun() {
    ...
    this.QQClipObject = this[bindingCQNode].utils.winrobot.QQClipObject;
    ...
    try {
      ...
      if (avatar) {
        ...
        if (newAvatar) {
          ...
          const cli = new this.QQClipObject();
          this.radio(`官推头像更新:${avatarUrl}\n`);
          cli.appendImg(path.resolve(process.cwd(), 'img', avatarFile))
            .end();
        }
      }
    } catch (err) {
  ...
  async request(msg, fromGroup) {
    ...
    if (index) {
      ...
      if (fromGroup === this.config.imageableGroup) {
        const cli = new this.QQClipObject();
        cli.appendText(`官推当前头像:${avatarUrl}\n`)
          .appendImg(path.resolve(process.cwd(), 'img', avatarFile))
          .end();
        return null;
      }
