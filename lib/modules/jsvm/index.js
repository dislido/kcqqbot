...
const pipelineOperator = require('@babel/plugin-proposal-pipeline-operator');
...
module.exports = class JSVM extends CQNode.Module {
  ...
  代码运行时间不能超过1s`,
    ...
    this.babelOption = {
      presets: [
        [presetEnv, { targets: { node: process.versions.node } }],
  ...
  onPrivateMessage({ msg }, resp) {
    if (msg.trim().startsWith('js')) {
      resp.send(this.runCode(msg.trim().slice(2)));
      return true;
    }
    return false;
  }
  runCode(js) {
    ...
    let extra = '';
    try {
      const runjs = babel.transform(decodeHtml(js), this.babelOption).code;
      const ctxGlobal = {
        ...this.context,
        console: {
          log(...msg) {
